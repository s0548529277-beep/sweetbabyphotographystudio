...
--
-- Every step below is written defensively (IF EXISTS / IF NOT EXISTS) so
-- this migration is safe to re-run if an earlier attempt only partially
-- applied.
...
WHERE w.booking_id = b.id AND w.user_id IS NULL;
...
ALTER TABLE public.photo_client_workflows DROP CONSTRAINT IF EXISTS photo_client_workflows_booking_id_key;
CREATE UNIQUE INDEX IF NOT EXISTS photo_client_workflows_booking_id_uniq ON public.photo_client_workflows (booking_id) WHERE booking_id IS NOT NULL;

CREATE INDEX IF NOT EXISTS photo_client_workflows_user_id_idx ON public.photo_client_workflows (user_id);
...
DROP POLICY IF EXISTS "photo_client_workflows_owner_select" ON public.photo_client_workflows;
CREATE POLICY "photo_client_workflows_owner_select" ON public.photo_client_workflows FOR SELECT TO authenticated
  USING (user_id = auth.uid());

DROP POLICY IF EXISTS "photo_client_images_owner_select" ON public.photo_client_images;
