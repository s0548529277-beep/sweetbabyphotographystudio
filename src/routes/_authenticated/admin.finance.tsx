...
const INCOME_CATEGORIES = ["צילומים", "השכרת סטודיו", "השכרת אביזרים", "מכירה", "אחר"];
...
function FinanceAdmin() {
  ...
  const [income, setIncome] = useState({ title: "", amount: "", category: "צילומים", received_on: new Date().toISOString().slice(0, 10) });
  ...
  const months = useMemo(() => {
    const map = new Map<string, { key: string; orders: number; bookings: number; manual: number; expenses: number }>();
    const get = (k: string) => {
      if (!map.has(k)) map.set(k, { key: k, orders: 0, bookings: 0, manual: 0, expenses: 0 });
      return map.get(k)!;
    };
    ...
    for (const mi of data.data?.manual ?? []) {
      get(monthKey(mi.received_on as string)).manual += Number(mi.amount ?? 0);
    }
  ...
  const totals = useMemo(
    () =>
      months.reduce(
        (acc, m) => ({
          income: acc.income + m.orders + m.bookings + m.manual,
  ...
  const maxBar = Math.max(1, ...months.map((m) => Math.max(m.orders + m.bookings + m.manual, m.expenses)));
    ...
    <div className="space-y-8" dir="rtl">
      ...
      <div className="bg-card rounded-2xl border border-primary/5 p-5">
        ...
        {data.isLoading ? (
          ...
          <div className="space-y-4">
            {months.map((m) => {
              const income = m.orders + m.bookings + m.manual;
              ...
              return (
                <div key={m.key} className="rounded-xl bg-cream/50 p-4">
                  ...
                  <div className="text-xs text-muted-foreground mt-2">
                    השכרת אביזרים {ils(m.orders)} · השכרת סטודיו {ils(m.bookings)} · הכנסות ידניות {ils(m.manual)}
                  </div>
                </div>
              );
            })}
      ...
      <div className="bg-card rounded-2xl border border-primary/5 p-5">
        ...
        <div className="grid sm:grid-cols-5 gap-3">
          ...
          <Input type="date" value={income.received_on} onChange={(e) => setIncome({ ...income, received_on: e.target.value })} />
        </div>
        ...
        <div className="mt-6 divide-y divide-primary/5">
          {(data.data?.manual ?? []).map((e: any) => (
            <div key={e.id} className="flex items-center justify-between py-2.5 text-sm">
              <div>
                <div className="font-medium">{e.title}</div>
                <div className="text-xs text-muted-foreground">
                  {new Date(e.received_on).toLocaleDateString("he-IL")} · {e.category}
                </div>
              </div>
              <div className="flex items-center gap-3">
                <span className="text-forest">{ils(Number(e.amount))}</span>
                <Button variant="ghost" size="icon" onClick={() => removeIncome(e.id)}>
