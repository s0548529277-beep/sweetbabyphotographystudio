...
// Every SKU shipped in the bundled seed catalog (studio-catalog.json). The
// live site (useCatalogCategories in catalog.ts) falls back to this bundled
// data for any SKU that has no row in the `items` table — so hard-deleting
// one of these SKUs' rows doesn't remove it from the site, it just clears
// the override and the bundled version comes right back. Deleting one of
// these must deactivate instead (see `del` below).
const BUNDLED_SKUS = new Set(STATIC_CATALOG.flatMap((c) => c.items.map((i) => i.sku)));
...
function ItemsAdmin() {
  ...
  const del = async (id: string) => {
    ...
    // A SKU that also exists in the bundled seed catalog must be
    // deactivated, not hard-deleted — deleting its row just removes the
    // override and the site's fallback logic resurrects the bundled
    // version immediately (this was reported live: "already deleted these
    // teddy bears several times and they keep coming back"). A row with no
    // bundled counterpart (created purely in /admin/items) has nothing to
    // resurrect from, so a real delete is safe there.
    const row = (items.data ?? []).find((i: any) => i.id === id);
    const isBundled = row && BUNDLED_SKUS.has(String((row as any).sku));
    const { error } = isBundled
