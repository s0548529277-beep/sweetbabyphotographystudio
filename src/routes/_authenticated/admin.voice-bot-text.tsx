...
const DTMF_MENU_STEPS: { key: string; title: string; detail: string }[] = [
  {
    ...
    detail:
      'קודם משמיע את מחירון הסטודיו ("תשובה קצרה — השכרת סטודיו" למטה — אפשר לערוך שם), ואז עובר לתהליך השריון הקבוע (אותו תהליך כמו "תהליך הזמנה קבוע — בהקלדה" למטה): שם (בקול) → תאריך (הקשה, יום+חודש בלבד, בלי שנה) → שעה (הקשה, 24 שעות) → משך (הקשה, 1-6) → מייל (בקול, אפשר לדלג) → סיכום ואישור בהקשה. אם השעה תפוסה, הבוט מבקש שעה אחרת באותו תאריך במקום סתם להיכשל.',
  },
