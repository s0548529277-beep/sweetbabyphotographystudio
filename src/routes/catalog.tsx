...
export const Route = createFileRoute("/catalog")({
  head: () => ({
    meta: [
      { title: "קטלוג אביזרים | Sweetbaby" },
      {
        name: "description",
        content:
          "קטלוג אביזרי צילום להשכרה - ניו-בורן, גיל שנה, משפחה וצילומי חוץ. סמנו את האביזרים ושלחו בקשה במייל.",
      },
      { property: "og:title", content: "קטלוג אביזרים | Sweetbaby" },
      { property: "og:description", content: "מבחר מלא של אביזרי צילום להשכרה." },
...
function CatalogPage() {
  ...
  const toggle = (sku: string) => setSelected((s) => ({ ...s, [sku]: !s[sku] }));
  ...
  const buildEmail = (): { subject: string; body: string } | null => {
    ...
    const lines = chosen.map((i) => `#${i.sku} - ${i.name} - ${i.price} ₪`).join("\n");
  ...
  const sendMailto = () => {
    const e = buildEmail();
    if (!e) return;
    window.location.href = `mailto:${EMAIL_TO}?subject=${encodeURIComponent(e.subject)}&body=${encodeURIComponent(e.body)}`;
  };

  const sendGmail = () => {
    const e = buildEmail();
    if (!e) return;
    const url = `https://mail.google.com/mail/?view=cm&fs=1&to=${encodeURIComponent(EMAIL_TO)}&su=${encodeURIComponent(e.subject)}&body=${encodeURIComponent(e.body)}`;
          ...
          <div className="catalog-box">
              ...
              <div className="cat-block" key={cat.title}>
                ...
                <div className="item-grid">
                  {cat.items.map((it) => {
                    ...
                    return (
                      <label key={it.sku} className={`item-card${on ? " checked" : ""}`}>
                        <input type="checkbox" checked={on} onChange={() => toggle(it.sku)} />
                        {it.img ? (
            ...
            <div className="cart-bar">
              ...
              <div className="cart-form">
                ...
                <button type="button" className="submit-btn" onClick={sendMailto}>
                  שליחה במייל
                </button>
                <button type="button" className="submit-btn gmail" onClick={sendGmail}>
                  שליחה דרך Gmail
                </button>
              </div>
            </div>
...
.sb-rental-cat .submit-btn.gmail { background:#d94235; color:#fff; }
