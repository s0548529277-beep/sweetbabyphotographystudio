...
import { isAdminVoiceCaller, verifyAdminPin, getAdminVoiceSnapshot, lookupAdminCustomerContact } from "./voice-admin.server";
...
- אם הלקוחה שואלת אם התקבל מייל ששלחה (למשל אישור תשלום/העברה), או על תכתובת קודמת — בקשי ממנה בקול את כתובת המייל המלאה, חזרי עליה לאישור (כתובות מייל קל לטעות בהן בטלפון), ורק אז קראי לכלי search_email. הקריאי לה תמצית קצרה בלבד ממה שנמצא, לא תוכן מלא.
...
- אם הכלים admin_business_snapshot/admin_open_door_now/admin_search_email/admin_read_email_body/admin_send_email/admin_lookup_customer מופיעים אצלך (זה קורה רק כשהמתקשרת היא בעלת הסטודיו, לפי המספר שממנו היא מתקשרת) — זו כנראה היא, לא לקוחה רגילה. אם היא מבקשת מידע עסקי, לפתוח את הדלת, לחפש/לשמוע מייל, לשלוח מייל, או **פרטי קשר של לקוחה ספציפית (טלפון/אימייל/שם)** — בקשי ממנה בקצרה קוד PIN לפני שאת קוראת לכלי המתאים (אף פעם לא בלי PIN), והשתמשי בדיוק במה שהכלי מחזיר. אם היא מבקשת לשמוע מייל ספציפי מתוך רשימת תוצאות חיפוש — קראי ל-admin_read_email_body עם ה-id המתאים. לפני שליחת מייל בפועל (admin_send_email) — חזרי בקול על הנמען, הנושא ותמצית התוכן לאישור. שאלה כמו "מה הטלפון של [שם]" או "תמצאי לי את הפרטים של [לקוחה]" — זה בדיוק admin_lookup_customer, אחרי PIN; לעולם אל תגידי שאין לך גישה לזה, יש. אל תציעי את היכולות האלה ללקוחה רגילה ואל תזכירי בכלל שהן קיימות אם הכלים לא מופיעים אצלך.`;
...
function buildVoiceTools(callerPhone: string) {
  ...
  return {
    ...
    // Customer-facing and deliberately narrow: unlike the PIN-gated
    // admin_search_email below (free-text, only offered to the studio
    // owner's own number), any caller can reach this one, so the Gmail
    // query is always built server-side from one email address (from:/to:
    // that address only) — never from free text the model chooses —
    // specifically so an unauthenticated caller can't use it to fish for
    // someone else's correspondence by name, phone number, or keyword.
    search_email: tool({
      description:
        "בודקת אם התקבל מייל מ/אל כתובת מסוימת בתיבת הסטודיו — למשל אישור תשלום שהלקוחה שלחה, או מייל קודם שלא נענה. יש להשתמש בזה רק אחרי שהלקוחה נתנה בעל פה כתובת מייל מלאה ומדויקת (חוזרים עליה בקול לאישור לפני החיפוש) — אסור לחפש לפי שם או טלפון בלבד.",
      inputSchema: z.object({ email: z.string().email().describe("כתובת המייל המלאה שהלקוחה אמרה ואישרה") }),
      execute: async ({ email }) => {
        try {
          const { searchGmailByEmail } = await import("@/integrations/google/gmail.server");
          const results = await searchGmailByEmail(email, 5);
          if (results.length === 0) {
            return {
              ok: true,
              found: false,
              message: "לא נמצאה תכתובת מהכתובת הזו. אפשר להציע שתשלח שוב, או לוודא שהכתובת נכתבה נכון.",
            };
          }
          return {
            ok: true,
            found: true,
            emails: results.map((r) => ({ subject: r.subject, date: r.date, snippet: r.snippet.slice(0, 200) })),
            message:
              "נמצאה תכתובת — תני ללקוחה בקול תמצית קצרה בלבד (בערך מתי ובאיזה נושא), בלי להקריא תוכן מלא או פרטים רגישים כמו קודי קופון או מספרי כרטיס.",
          };
        } catch (e: any) {
          return { ok: false, message: `חיפוש המייל נכשל: ${e?.message ?? "שגיאה לא צפויה"}. הציעי שהסטודיו יבדוק ידנית.` };
        }
      },
    }),
    ...
    ...(isAdminVoiceCaller(callerPhone)
      ? {
          ...
          admin_lookup_customer: tool({
            description:
              'מחפשת פרטי קשר (שם/טלפון/אימייל אם קיים) של לקוחה ספציפית לפי שם או טלפון, מתוך שריונים, הזמנות אביזרים והזמנות ניו-בורן — לבעלת הסטודיו בלבד. יש לוודא PIN תקין *לפני* קריאה לכלי, בדיוק כמו ב-admin_business_snapshot (אם כבר אומת PIN קודם באותה שיחה, אפשר להעביר את אותו קוד בלי לבקש שוב). זה בדיוק הכלי לשאלה כמו "מה הטלפון של X" או "תמצאי לי את הפרטים של Y" — אל תגידי שאין לך גישה למידע הזה, יש. אם יש כמה תוצאות תואמות (למשל אותו שם הזמין כמה פעמים), הקריאי בקצרה את כולן עם התאריך של כל אחת כדי שהיא תזהה את הנכונה. אימייל יהיה ריק אצל לקוחות שהזמינו כאורחות בלי חשבון — זה נורמלי, לא תקלה.',
            inputSchema: z.object({
              pin: z.string().min(1).describe("קוד ה-PIN שהמתקשרת אמרה בקול"),
              query: z.string().min(1).describe("שם (מלא או חלקי) או מספר טלפון של הלקוחה המבוקשת"),
            }),
            execute: async ({ pin, query }) => {
              if (!verifyAdminPin(pin)) {
                return {
              ...
              try {
                const matches = await lookupAdminCustomerContact(query);
                return matches.length === 0
                  ? { ok: true, matches: [], message: `לא נמצאה לקוחה תואמת ל"${query}". הציעי שתבדוק בממשק הניהול, יכול להיות שהשם/הטלפון שונה קצת.` }
                  : { ok: true, matches };
              } catch (e: any) {
                return { ok: false, message: `החיפוש נכשל: ${e?.message ?? "שגיאה לא צפויה"}. הציעי שתבדוק בממשק הניהול במקום.` };
              }
