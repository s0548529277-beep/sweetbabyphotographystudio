 ...
 * backgrounds/floors), transcribed from its real slide deck — embedded in
 * SYSTEM below so the chat/voice assistant can answer real troubleshooting
 * questions, and also exported standalone so the phone menu's "read me the
 * whole guide" option (voice-menu.server.ts) can recite it verbatim without
 * duplicating the text. Declared before SYSTEM since SYSTEM's template
 * literal interpolates it — a const used before its own declaration in the
 * same module throws (temporal dead zone), so order here matters.
 */
...
9) שאלה/תקלה שלא מכוסה כאן — תציעי בחום להתקשר: 054-8529277.`;
...
- הדרכה עצמאית לשימוש בסטודיו: קיימת גם מצגת תמונות + סרטון הדרכה מלאים, נגישים ללקוחה מיד אחרי סגירת ההזמנה (בעמוד סיום התשלום ובחשבון שלה) — אפשר להפנות אליהם לצפייה מלאה. אבל יש לך גם את התוכן המלא של ההדרכה עצמה למטה (בסעיף "הדרכה לשימוש בסטודיו בפועל") — אם לקוחה שואלת שאלה ספציפית או מדווחת על תקלה (הפלאש לא הבזיק, איך מחברים את המשדר, איזה רקע מותר לדרוך עליו וכו') תני לה תשובה ישירה ומדויקת מהתוכן הזה, לא רק הפניה לעמוד.
...
הדרכה לשימוש בסטודיו בפועל (מהמצגת הרשמית של הסטודיו — תוכן אמיתי, אפשר לצטט ממנו במדויק):
${STUDIO_GUIDE_HE}`;
