...
- קודי קופון: משתנים מדי פעם — לעולם אל תזכיר קוד מהזיכרון, תמיד תבדוק עם list_active_coupons ותן קוד אמיתי ועדכני. אל תציעי קוד קופון ביוזמתך כ"בונוס" — רק כשהלקוחה שואלת במפורש על הנחה, קופון או "אפשר יותר זול".
...
export const chatWithBot = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const personalized = data.isAuthenticated
      ...
      : `\n\nהמשתמשת לא מחוברת. לביצוע הזמנה בפועל הציעי לה להתחבר בעמוד /auth — אבל בדיקת זמינות אפשר לעשות גם בלי התחברות. אם עולה נושא הנחה/מחיר יותר זול ואין קופון כללי מתאים מ-list_active_coupons (או שהיא שואלת אם יש הנחה) — הציעי לה בעדינות להירשם לניוזלטר (יש טופס בתחתית כל עמוד באתר) כדי לקבל קוד הנחה אישי לברוכה הבאה במייל. אל תזכירי קוד קונקרטי של הניוזלטר בעצמך — הוא נשלח אוטומטית רק אחרי הרשמה אמיתית.`;
...
- קוד קופון — לעולם אל תמציאי, תמיד תבדקי עם list_active_coupons. אל תיזמי הצעת קופון מעצמך (כ"בונוס" או בסוף תשובה על מחיר) — רק כשנשאלת ישירות על הנחה/קופון/"אפשר יותר זול".
- כששואלים על מחיר, הנחה או "אפשר יותר זול" — חשבי את המחיר המדויק בכלי המתאים, ובדקי גם list_active_coupons לפני התשובה. אל תבטיחי הנחה שלא הוחזרה מהכלי. אם list_active_coupons לא מחזיר כלום ללקוחה לא-רשומה — הציעי ניוזלטר במקום (ראי הנחיה למעלה).
