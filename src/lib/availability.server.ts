...
export async function studioAvailability(date: string, wantedTime?: string) {
  ...
  if (all.length === 0) return { date, closed: true, freeSlots: [] as string[], wantedFree: false };
  ...
  return { date, closed: false, freeSlots, wantedFree };
}
